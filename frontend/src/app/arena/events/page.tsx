...
async function loadData() {
  try {
    const [stats, meta] = await Promise.all([
      apiGet<Resp>("/stats/arena/augments?with_rarity=false&min_games=30&limit=200"),
...
export default async function ArenaEvents() {
  const { stats, meta, error } = await loadData();

  return (
    <main className="container mx-auto max-w-6xl px-4 py-8">
      <header className="mb-4">
        <div className="text-xs text-text-dim">競技場 · 事件選擇</div>
        <h1 className="mt-1 text-2xl font-bold tracking-tight">事件選擇排行</h1>
        <p className="mt-1 text-sm text-text-muted">
          特殊事件回合的選擇(無稀有度),例:獲得鐵砧、修復增幅、等級增幅等。選擇看情境,跟英雄關聯較弱。
        </p>
      </header>

      <ArenaTabs active="events" />
